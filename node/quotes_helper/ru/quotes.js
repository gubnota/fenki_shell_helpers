...
for (let i = 0; i < files.length; i++) {//
  let file = files[i][0];//first file in the list to read author's name
  ...
  if (author_id > 0) //author is found! // && author_id == 888
  {
    ...
    for (let j = 0; j < files[i].length; j++) {
      ...
      root.querySelectorAll('.blockquote-text a').forEach(el => {
        var phrase = el.innerHTML.trim();
        if (phrase.length < 400 && h.isRussian(phrase)) {//&& phrase.search(/Ç|ç|É|é|È|è|Ê|ê|Ë|ë|Î|î|Ï|ï|Ô|ô|Œ|œ|Ù|ù|Û|û|Ü|ü|Ÿ|ÿ/) > -1 && !phrase.includes(" of ") && !phrase.includes(" the ") && !phrase.includes(" was ") && !phrase.includes(" to ") && !phrase.includes(" by ") && !phrase.includes(" you ") && !phrase.includes("I ") && !phrase.includes(" is ")
